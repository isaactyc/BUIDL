...
class TxIn:
    ...
    #this turns the unique pairing into a string, and later byte string
    @property
    def spend_message(self):
        #turns the transaction into a string using f, and a byte string with encode
        return f"{self.tx_id}:{self.index}".encode()
    #outpoint is a "point" that directs us to a specific output, first referencing the transaction, as well as a particular input/output within the transaction
    @property
    def outpoint(self):
